...
class TestRuntimeMode:
    ...
    def test_coding_instructions_append_their_own_block(self, tmp_path):
        _git_init(tmp_path)
        cfg = {"agent": {"coding_context": "on", "coding_instructions": "Always run /clean before commit."}}
        ...
        assert any("Always run /clean before commit." in b for b in blocks[1:])

    def test_coding_instructions_accept_a_list(self, tmp_path):
        _git_init(tmp_path)
        cfg = {"agent": {"coding_context": "on", "coding_instructions": ["No tsc/lint on UI.", "Run /clean."]}}
        mode = cc.resolve_runtime_mode(platform="cli", cwd=tmp_path, config=cfg)
        instr_block = mode.system_blocks()[-1]
        assert "No tsc/lint on UI." in instr_block
        assert "Run /clean." in instr_block
