...
class TestGetPreStopContinueMessage:
    """`pre_stop` directive aggregation — mirrors the pre_tool_call block path."""

    def test_continue_canonical(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [{"action": "continue", "message": "run /clean"}],
        )
        assert get_pre_stop_continue_message(session_id="s") == "run /clean"
    ...
    def test_claude_block_means_continue(self, monkeypatch):
        ...
        assert get_pre_stop_continue_message() == "run the formatter"
    ...
    def test_first_actionable_directive_wins(self, monkeypatch):
        ...
        assert get_pre_stop_continue_message() == "second"
    ...
    def test_message_is_trimmed(self, monkeypatch):
        ...
        assert get_pre_stop_continue_message() == "tidy up"
    ...
    def test_invalid_returns_ignored(self, monkeypatch):
        ...
        assert get_pre_stop_continue_message() is None
